...
def _cholesky_update_gpu_lowering_rule(platform, ctx, r_matrix, w_vector):
  ...
  rule = ffi.ffi_lowering(f"{platform}_cholesky_update_ffi",
...
def _generic_lu_pivots_to_permutation(swaps, permutation_size):
  ...
  permutation = lax.broadcasted_iota(jnp.int32, batch_dims + (m,),
                                     len(batch_dims))
  if m == 0 or is_constant_dim(k) and k == 0:
    return permutation
...
def _lu_cpu_gpu_lowering(getrf_impl, ctx, operand, *, platform: str):
  ...
  if jaxlib_version < (0, 4, 32) or ctx.is_forward_compat():
    ...
    if platform == "cpu":
      target_name = lapack.build_lapack_fn_target("getrf", operand_aval.dtype)
      target_name += "_ffi"
    else:
      target_name = f"{platform}solver_getrf_ffi"
