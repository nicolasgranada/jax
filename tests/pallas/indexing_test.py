...
class AdvancedIndexerOpsTest(PallasBaseTest):
  ...
  def test_advanced_indexer(self, in_shape: tuple[int, ...], indexer_str: str):
    a, b, c, d = self.a, self.b, self.c, self.d  # pylint: disable=unused-variable

    x = jnp.arange(np.prod(in_shape), dtype=jnp.float32).reshape(in_shape)
    indexer = eval(f"sb[{indexer_str}]")  # pylint: disable=eval-used
    y = x[indexer]
...
'''
    ...
    exec(func_str, globals())  # pylint: disable=exec-used

    y_ = self.pallas_call(
        kernel,  # pylint: disable=undefined-variable
